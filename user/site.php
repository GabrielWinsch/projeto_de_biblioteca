...
if(isset($_GET['pesquisa'])){
    ...
    $sql = "SELECT * FROM obras as o LEFT JOIN obras_has_autores as oa ON o.cod_obra = oa.Obras_cod_obra 
    LEFT JOIN autores as a ON oa.Autores_cod_autor = a.cod_autor 
    WHERE (o.tit_obra LIKE '%$pesquisa%' OR
    a.nome_autor LIKE '%$pesquisa%') AND 
    o.reservada = 'N' AND o.emprestada = 'N'
    ORDER BY o.tit_obra";
    ...
    if(mysqli_num_rows($limite) > 0){
      ...
      mysqli_close($conn);
        ...
        echo "<script type='text/javascript'>alert('Nenhuma obra corresponde a pesquisa');window.location = 'site.php'</script>";
  }
/*else{
    $sql ="SELECT * FROM obras as o 
    LEFT JOIN obras_has_autores as oa ON o.cod_obra = oa.Obras_cod_obra 
    LEFT JOIN autores as a ON oa.Autores_cod_autor = a.cod_autor 
    WHERE o.reservada = 'N' AND o.emprestada = 'N' 
    ORDER BY o.tit_obra
    LIMIT 10;";
    $result=mysqli_query($conn, $sql);
...
<div id="content">
                <div id="tabelaUsuarios">
                    ...
                    <div class="center">
                    <td><a class="brand-text" href="site.php?<?php if(isset($pesquisa)){echo "pesquisa=$pesquisa&";}?>pagina=<?php if($pc>1){echo $pc -1;}else{echo $pc;};?>">Anterior <--</a></td>
                    <td><a class="brand-text"><?php echo "pág $pc de $tp" ?></td>
                    <td><a class="brand-text" href="site.php?<?php if(isset($pesquisa)){echo "pesquisa=$pesquisa&";}?>pagina=<?php if($pc<$tp){echo $pc +1;}else{echo $pc;};?>">--> Proxima</a></td>
                    </div>
                    ...
                    <table id="tabela">
						...
						<?php foreach($obras as $obra): ?>
                            <tr>
								...
								<td><a class="brand-text" href="reserva.php?id=<?php echo $obra['cod_obra']?>">Reservar</a></td>
                            </tr>
						<?php endforeach; ?>
                        </tbody>            
                    </table>
